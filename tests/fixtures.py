...
def _test_credential():
    ...
    SecurityKey.set_challenge(session, base64url_to_bytes("CeTWogmg0cchuiYuFrv8DXXdMZSIQRVZJOga_xayVVEcBj0Cw3y73yhD4FkGSe-RrP6hPJJAIm3LVien4hXELg"))
...
def _test_auth_credential():
    ...
    # update challenge to fit the test-credential below
    SecurityKey.set_challenge(session, base64url_to_bytes("iPmAi1Pp1XL6oAgq3PWZtZPnZa1zFUDoGbaQ0_KvVG1lF2s3Rt_3o4uSzccy0tmcTIpTTT4BU1T-I4maavndjQ"))
...
def _security_key(passwordless_login=False):
    ...
    key = SecurityKey.verify_registration(user, session, cred, passwordless_login=passwordless_login)
